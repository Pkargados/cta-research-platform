...
## Phase 11 — Relative Value spreads & Seasonality 🟢 11b/11c built 2026-07-31, 11d built 2026-08-01
...
**Real gap found 2026-08-01, before any build work started**: `Time_Series_Models.ipynb`
— the notebook this plan's own second paragraph and CLAUDE.md's current-state table both
cite as already "rebuilt" with real rolling-window Engle-Granger results (Corn/Wheat,
Gold/Silver, Brent/WTI tested, Brent/WTI cointegrated in 45% of windows) — **does not
exist anywhere in the current repo**. Checked directly: no `.ipynb` file at the repo root
or any shallow subdirectory, not gitignored (only `.ipynb_checkpoints/` is), and `git log
--all` shows no commit ever added a file by that name except the very first whole-repo
snapshot commits (`01d0eb5`/`9ef6161`) — only `deprecated/Scripts/Time_Series_Models_
legacy.ipynb` (the OLD, full-sample-cointegration, pre-rebuild version CLAUDE.md's own
Hard Rule 2 flags as the exact look-ahead bug this rebuild was supposed to fix) is
actually present. Per this project's own source-of-truth ordering
(`notebooks/*.ipynb` and `Data/*`, in that order, over any Markdown claim), **the
documented "Foundation done (2026-07-14)" status for Phase 2d cointegration is not
verified against source and should not be treated as a starting point** — the rolling
Engle-Granger work (including the 45% Brent/WTI figure) needs to be REBUILT from
scratch, not resumed, before any of this section's own signal-construction plan can be
implemented on top of it. `src/` confirmed to have zero cointegration/spread/Kalman-
filter modules today (`src/signals/` only has breakout/carry/combine/crossover/momentum/
seasonality/short_term_reversal/transforms/value/vix_overlay/xs_momentum.py) — this is a
genuinely greenfield build, not a port of existing tested code. `statsmodels` (0.14.4,
already a dependency) has `coint` (Engle-Granger) ready to use; no Kalman filter library
is installed (`pykalman`/`filterpy` both absent) — implement directly (a standard 2-state
[alpha, beta] pairs-trading Kalman filter is ~30-50 lines of numpy, matching this
project's own preference for direct implementation over a new dependency, e.g. the
from-scratch Gerber statistic) rather than adding a new dependency, unless a build
session decides otherwise. CLAUDE.md's current-state table corrected same day to
reflect this — see that table's own Classic cointegration row.

### 11d. Relative Value sleeve — built 2026-08-01

Three open decisions confirmed before writing code (per direct instruction): crack
spread crude leg = **WTI** (NYMEX 3:2:1 convention); entry/exit rule = **both**
continuous vol-scaled z-score AND discrete threshold band, tested as parallel specs,
no headline pick (not the plan's own lean toward continuous-only); Kalman filter =
**implemented directly** in `src/signals/hedge_ratio.py` (`pykalman`/`filterpy`
confirmed absent).

**New modules**: `src/signals/cointegration.py` (`rolling_engle_granger`,
`fraction_cointegrated`, `rolling_cointegration_report` — strictly trailing-window,
CLAUDE.md Hard Rule 2), `src/signals/hedge_ratio.py` (`fixed_ratio`,
`rolling_ols_beta`, `kalman_hedge_ratio`), `src/signals/relative_value.py` (spread
construction for all three groups, `zscore_spread_signal`,
`continuous_zscore_signal`, `threshold_band_signal`, per-pair builders). `research/
relative_value_cointegration_check.py` (diagnostic driver), `research/relative_value.
py` (standalone 7-pair bake-off), `research/relative_value_book.py` (pooled Book),
`research/relative_value_gerber.py` (covariance-estimator comparison).

**Rolling cointegration diagnostic — real numbers, not the old unverified claim**:
tested all 6 two-leg pairs (crack spread excluded — a fixed economic ratio, not a
statistically estimated relationship, see `signals.cointegration`'s own docstring),
252-day window/21-day step. Every pair came back WEAK-TO-NO evidence: wti_brent
8.7%, gold_silver 7.6%, platinum_palladium 3.3%, wheat_kcwheat 6.4%, corn_wheat
4.3%, rbob_heatingoil 11.0% of rolling windows reject the null at alpha=0.05 — not
the old `Time_Series_Models.ipynb` claim of 45% for Brent/WTI (that notebook never
existed, see the "Real gap found 2026-08-01" addendum above). Sanity-checked
directly, not a bug: even a full-sample EG test on WTI-Brent log prices gives
p=0.62; spot-checked individual 252-day windows independently and got the same
answer. Plausible explanation, not chased further: WTI-Brent's real regime breaks
over 2010-2026 (shale boom, 2015 US export-ban repeal) — a single EG test isn't
built to handle a relationship that shifts structurally mid-sample. This diagnostic
does NOT gate which pairs get built (already decided on economic/structural
grounds) or the signal's daily trading logic (continuous, always-on, same
"binary underperforms continuous" discipline as every other signal family) — it's
reported alongside the backtest, not used to filter it.

**Real bug found and fixed live, the same sparse-calendar failure mode this
project has now hit four times** (crossover, Yang-Zhang, `portfolio.covariance`,
now this): `rolling_ols_beta`'s and `zscore_spread_signal`'s first versions used
`min_periods == window` (require the ENTIRE trailing window non-null). Checked
directly: Corn/Wheat's longest jointly-valid consecutive streak in the whole
~15-year panel is only 70 days — nowhere near a 252-day window — so the rolling
OLS beta estimate came back 100% NaN, not just reduced coverage. Fixed by reusing
the already-validated `MIN_FRAC=0.7` tolerance (same value, same reasoning, every
prior instance). A SECOND, lower tolerance (`VOL_MIN_FRAC=0.5`) was needed
specifically for `realized_vol`, which operates on `spread_return` — a DIFFERENCED
series, structurally sparser than the level it's built from (checked directly:
corn_wheat's spread level is ~96% dense but its day-over-day diff is only ~64%,
since one isolated missing day corrupts two diff values, not one; wheat_kcwheat's
diff density is only ~50%). Applying the level-calibrated 0.7 tolerance to
`realized_vol` left it almost entirely NaN (corn_wheat: 626 of 4170 dates;
wheat_kcwheat: 8 of 5019) before this second fix.

**Standalone bake-off — 7 pairs x 2 sizing rules, daily frequency first** (decided
the same way breakout's own daily-direction choice was decided, before looking at
any result): turnover came back very high across the board (9x-106x annualized,
worse than every family except short-term reversal), net-of-cost consistently well
below gross. **Weekly frequency added as a genuine second parallel spec, per
direct instruction, after seeing this** — NOT selected after comparing which wins
(both reported, 28 rows total). Weekly generally cut turnover 2-10x and often
(not always) improved net Sharpe — e.g. wti_brent test net Sharpe -0.53 (daily) vs.
+0.58 (weekly); rbob_heatingoil validation net Sharpe +1.30 (daily) vs. +0.03
(weekly, the one case where daily clearly won).

**Hedge-ratio method bake-off** (Corn-Wheat, RBOB-HeatingOil, validation Sharpe of
the continuous-daily spec): Kalman won both — decisively for RBOB-HeatingOil
(val Sharpe 1.52 vs. rolling-OLS's -0.20), narrowly for Corn-Wheat (both weak:
Kalman -0.68 vs. rolling-OLS's -9.48, the latter driven by a near-collapse in one
narrow window, not a stable comparison).

**Standalone result, net-of-cost Sharpe (train/validation/test), best sizing x
frequency per pair**: rbob_heatingoil is the clear standout (+0.09/+1.30/+0.95,
daily-continuous — positive in every period). Everything else is weak-to-negative:
wti_brent (-0.63/+0.07/+0.58, weekly), gold_silver (+0.35/-0.44/+0.12,
weekly-threshold), crack_spread (-0.13/-0.19/+0.03, threshold), platinum_palladium
(-0.21/+0.01/-0.25, weekly-threshold), wheat_kcwheat (-0.13/+0.23/-0.02,
weekly-continuous) — all weak/mixed, not catastrophic. corn_wheat is the one
outright disaster (-2.26/-3.09/-4.45, daily-continuous, Kalman beta) — a genuine
finding, not a bug: its z-score has a persistent +1.19 mean rather than centering
on 0 (checked directly), meaning the spread TRENDS rather than mean-reverts over
this window — consistent with, not contradicted by, its own 4.3% cointegration
reading above. This is the second outright-unprofitable standalone result in this
project after short-term reversal, for a related reason (a mean-reversion bet
against an object that doesn't actually mean-revert).

**Pooled Relative Value Book** (`single_strategy_portfolios.build_book`'s
calibration reused directly — GAMMA/KAPPA/LAMBD/TARGET_VOL_BOOK/MAX_WEIGHT/
COV_WINDOW/COV_FREQ/EWMA_HALFLIFE, weekly cadence, per CLAUDE.md Rule 6). Per-pair
entry/exit rule picked on validation NET Sharpe at weekly frequency (not gross —
turnover/cost is this sleeve's central problem, unlike Trend/Carry's own
gross-selected flavor bake-off): wti_brent/wheat_kcwheat/crack_spread ->
continuous; gold_silver/platinum_palladium/corn_wheat/rbob_heatingoil ->
threshold. `single_strategy_portfolios._active_columns`'s 90% valid-return-density
gate (reused verbatim, not loosened) admits only 4 of 7 pairs: wti_brent (92%
density), gold_silver (96%), rbob_heatingoil (99.6%), crack_spread (99.6%).
platinum_palladium (87%), corn_wheat (68%), wheat_kcwheat (50%) are excluded — a
genuine cross-commodity trading-calendar sparsity finding (these legs don't trade
on exactly the same days), not a construction bug, and not forced in by loosening
the gate. **Result, net-of-cost Sharpe: train -0.254, validation +0.345, test
+0.505** — positive in validation and test despite 3 of the 4 active pairs being
weak-to-negative standalone, a real illustration of the plan's own diversification
thesis (many small, low-correlation spread bets). Turnover 0.102 per rebalance
(~5.3x annualized at weekly cadence) — much lower than any individual pair's own
standalone turnover, consistent with the Book's own position-inertia (`kappa`) and
vol-targeting mechanics smoothing the combined path. 690 valid rebalance dates, 1
stale gap (`max_gap_days`, unremarkable).

**Gerber statistic covariance vs. Ledoit-Wolf, closing the loop on the session's
original motivating question** (see the Gerber statistic covariance row's own
revised RV prior below — Ledoit-Wolf was EXPECTED to win here, for a sharper reason
than the original Trend/Carry framing): net-of-cost Sharpe (train/validation/test)
— Ledoit-Wolf -0.254/+0.345/+0.505; gerber_c05 -0.062/+0.156/+0.818; gerber_c07
-0.101/+0.106/+0.826; gerber_c09 -0.071/+0.107/+0.851. **Genuinely mixed, not a
clean confirmation of the prior**: Gerber clearly helps TEST Sharpe (0.82-0.85 vs.
Ledoit-Wolf's 0.50 — the opposite direction from what the prior predicted) but
clearly hurts VALIDATION (0.11-0.16 vs. 0.34) and roughly doubles both turnover
(0.194 vs. 0.102) and max drawdown (-0.12/-0.13 vs. -0.058) at every threshold.
**Ledoit-Wolf remains the default** — no single-period improvement is enough to
override a worse validation result and materially worse risk/turnover profile,
same standard this project has applied to every other Gerber-vs-Ledoit-Wolf
Book-level comparison (Phase 7's Gerber section).

**Dashboard**: `dashboard/pages/26_relative_value_performance.py` (new "Strategy
Performance" group entry) — cointegration diagnostic table, standalone bake-off
(pair/spec/frequency/gross-net selectors), pooled-Book tearsheet with attribution,
covariance-estimator comparison table. Verified exception-free via
`streamlit.testing.v1.AppTest` across all 27 pages (up from 26). 25 new unit tests
(`tests/test_cointegration.py`, `tests/test_hedge_ratio.py`,
`tests/test_relative_value.py`) — 335 passing project-wide (up from 310).
...
|---|---|---|---|
| 1 | Rebuild vs. relabel `Time_Series_Models.ipynb` | Phase 1 | **This row's own "Done 2026-07-14" claim was itself wrong — corrected 2026-08-01.** Checked directly before starting the §11d Relative Value build: no `.ipynb` by this name, or any rebuilt rolling-cointegration notebook, was ever added to this repo (`git log --all` confirms) — the "33% of windows, regime-dependent" figure this row previously cited was never verified against real code. **Actually done 2026-08-01**, as real `src/`/`research/` code (not a notebook, per this project's own "new research work happens in `research/*.py`" rule) — `src/signals/cointegration.py`, `research/relative_value_cointegration_check.py`. Real result: weak-to-no cointegration evidence for every pair tested (3.3-11.0% of rolling windows at alpha=0.05), not 33% or the later-cited 45% Brent/WTI figure either — see §11d's own "built" writeup for full numbers. |
...
| 14 | Relative Value sleeve's three open construction questions (§11d plan's own "needs deciding during implementation" list): crack spread crude leg, entry/exit rule, Kalman-vs-new-dependency | §11d build, before any RV code was written | **Confirmed by direct instruction 2026-08-01, before writing code.** Crack spread crude leg = **WTI** (NYMEX 3:2:1 convention, matches this project's existing WTI-centric precedent). Entry/exit rule = **both** continuous vol-scaled z-score AND discrete threshold band, tested as parallel specs with no headline pick — NOT the plan's own lean toward continuous-only, a genuine departure per direct instruction ("test both"). Kalman filter = **implemented directly** in `src/signals/hedge_ratio.py` (`pykalman`/`filterpy` confirmed absent), not a new dependency. See §11d's own "built 2026-08-01" section for the full result these decisions led to. |
