"""
databento/transform_databento.py — Raw Databento GLBX.MDP3/IFUS.IMPACT
`ohlcv-1d` + `definition` archives (`Data/databento_raw/*.zip`) -> the 6
`Data/term_structure*.parquet` output tables + `Data/databento_transform_manifest.csv`.

Rebuilt from `databento/DATA_QUALITY_REPORT.md` (the full 42-asset exploration
log) after the 2026-07-24 data-loss incident destroyed the original script with
no recoverable copy — the raw archive and every output parquet survived (both
live in `Data/`, untouched), so this rebuild was validated directly against the
real, already-computed output rather than only against prose. See
`RECONSTRUCTION_PROGRESS.md` Phase 5 for the validation methodology and the
specific real examples (LE calendar spreads, LE-HE cross-commodity, CL:BZ,
SOFR's :AB/:DF/:BB/:SB) used to reverse-engineer the exact leg-resolution
algorithm empirically before writing any parsing code.

Tooling: polars end-to-end — read, join, filter, leg-resolution, AND the
final merge-with-existing-data + parquet write — matches the report's own
documented decision (measured 6.8x speedup at the 42-asset scale, Section 6)
and stays scoped to `databento/` only (`cleanup.md`'s pandas-elsewhere
convention). No pandas dependency anywhere in this module: polars writes a
standard Parquet file, fully readable by every downstream pandas-based
consumer in this project (`data.term_structure`, etc.) with no behavior
change — a Parquet DATE32 column round-trips into pandas as `datetime64[ns]`
automatically. Only the manifest CSV (`databento_transform_manifest.csv`)
uses pandas, matching every other manifest file's convention project-wide.

Root/exchange mapping is NOT duplicated here — reused directly from
`jobs/capture_term_structure.py`'s own `UNIVERSE` dict, since that module's
`contract_symbol` convention (`{root}{month_code}{2-digit-year}.{exchange}`,
confirmed directly against the real `Data/term_structure.parquet` output) is
exactly the identity this transform must also produce, for the "Databento
wins on overlap" dedup (CLAUDE.md) to actually collide correctly between the
two sources.

Key findings this rebuild re-derived and validated empirically against real
data (not re-guessed from prose alone):
- The raw `symbol`/`raw_symbol` field is decade-ambiguous (single digit year);
  `definition`'s `maturity_year`/`maturity_month` fields are the fix, for
  OUTRIGHTS directly and for combo (spread/butterfly/condor/average/pack)
  instruments as the anchor leg's absolute (year, month) — confirmed directly
  against real LE/HE spread instrument definition rows: a combo instrument's
  own `maturity_year`/`maturity_month` always matches whichever leg is
  chronologically NEAREST (front), regardless of which root that leg belongs
  to (checked on a real LE-HE cross-commodity spread, where the anchor was the
  HE-side leg despite the query being against LE's own parent symbol).
- Every other leg's absolute year = anchor_year + ((leg_digit - anchor_digit)
  mod 10) — legs are always listed at or after the anchor chronologically, so
  this always resolves to a small non-negative offset.
- A 2-digit trailing year (e.g. `U36`) is an unambiguous absolute year
  (`2000 + yy`) needing no anchor at all — WTI's real far-dated (>2035) legs.
...
- Outright rows get an EXACT-zero (not merely non-positive) filter on any of
  open/high/low/close — preserves genuine negative prints (the real WTI
  April-2020 settlement) while dropping incomplete/fabricated bars (ICE's
  documented pattern). Applied to outrights only — spreads/butterflies/etc.
  legitimately trade negative.
- ICE assets (`IFUS.IMPACT`, this project's 5 softs) have a second,
  non-representative `_Z`-suffixed instrument per contract month, excluded by
  `raw_symbol` suffix, and use a structurally different `leg_instrument_id`-
  based spread/butterfly schema this transform does not attempt to resolve
  (logged and skipped, not guessed).
"""
...
RAW_DIR = Path(__file__).resolve().parent.parent / "Data" / "databento_raw"
...
# `datetime.date` values, not `datetime64[ns]` (confirmed directly,
# not assumed) -- every downstream consumer in this project expects a
# real pandas Timestamp column, so dates are carried as Datetime(ns)
# throughout this module, not Date, even though only the date (never
# time-of-day) is ever meaningful here.
...
# ---------------------------------------------------------------------------
# Raw file I/O -- in-memory zst decompression (no filesystem extraction: a
# real, measured ~30x speedup over `zipfile.extractall()` at this file count,
# since polars auto-detects zstd from the byte stream and per-file filesystem
# writes are what actually dominates wall time at ~5,000 files/zip, not parsing)
# ---------------------------------------------------------------------------
...
def _read_zip_csvs(zip_path: Path, columns: list) -> pl.DataFrame:
    ...
    boundary ambiguity; matches the report's own `file_date` terminology)."""
    frames = []
    with zipfile.ZipFile(zip_path) as zf:
        ...
        for name in names:
            raw = zf.read(name)
            try:
                df = pl.read_csv(io.BytesIO(raw), columns=columns, infer_schema_length=0 if False else None)
            except Exception:
                continue
            if df.height == 0:
                continue
...
# their last real print (found live on FeederCattle/SOFR -- see module
# docstring's linked WORKFLOW.md history). raw_symbol/instrument_class/
# maturity_year/maturity_month are immutable for a given instrument_id's
# whole life, so reusing an OLDER definition record is exactly as correct as
# an exact-date one, not a guess.
# ---------------------------------------------------------------------------
...
def filter_ice_z_variant(df: pl.DataFrame) -> tuple:
    ...
    outright, true meaning unconfirmed) -- excluded by suffix."""
    before = df.height
...
# ---------------------------------------------------------------------------
# Leg resolution -- the anchor-leg-plus-modular-offset algorithm, validated
# directly against real LE/HE/CL/BZ/SOFR examples (module docstring).
# ---------------------------------------------------------------------------
...
def _combo_pl_df(rows: list, columns: list) -> pl.DataFrame:
    """Build a polars DataFrame from the per-row combo-parsing loop's plain
    Python dict list, with an explicit column order/schema so an EMPTY list
    still produces a correctly-typed zero-row frame (polars can't infer a
    schema from nothing), matching real data's dtypes closely enough that a
    later concat against a populated frame never hits a schema mismatch."""
    if not rows:
...
def _parse_single_leg(token: str, default_root: str = None):
    """Parse one leg token into (root, month_code, digit_str, explicit_2digit).
    `token` is either bare `{month_code}{1-2 digit}` (root inherited from
...
def _resolve_absolute_year(digit_str: str, anchor_year: int, anchor_digit: int) -> int:
    ...
    interpretation -- confirmed directly, WTI's own far-dated legs). A
    1-digit code is resolved as a non-negative modular offset from the
    anchor leg -- legs are always listed at or after the anchor
    chronologically."""
    if len(digit_str) == 2:
...
def _root_exchange(root: str):
    ...
    but untracked product (e.g. MCL, WS, HH, GE, SYP -- logged as
    spread_unknown_root_exchange and skipped, never guessed)."""
...
# Vectorized replacement for the outright table's per-row `map_elements`
# lookup: `maturity_month` (1-12) -> its single-char month code, joined
# rather than called via a Python callback per row.
...
def resolve_legs(legs, maturity_year: int, maturity_month: int):
    ...
    maturity_month (confirmed empirically: a combo's maturity always tracks
    its chronologically nearest leg, regardless of that leg's root). Returns
    a list of (root, month_code, absolute_year), same order as input, or
    None if no leg matches (never guessed).
    ...
    each candidate is tried as the anchor and the one producing the
    SMALLEST total year-span across all legs wins. A real near-dated combo
    instrument's legs are never many years apart, so the tightest-span
    resolution is the economically sensible one -- a wrong anchor choice
    here (confirmed live on a real LE-HE spread, `LEQ5-HEQ4`: naively
    anchoring on the first-listed leg put the far leg 9 years out instead
    of 1) always produces an implausibly wide span, which this tie-break
    catches and rejects."""
...
def _legs_to_symbols(resolved_legs, unknown_root_counter):
...
def _resolve_combo_key(raw_symbol: str, maturity_year: int, maturity_month: int, root: str) -> dict:
    ...
    safe to call once per UNIQUE key rather than once per row (confirmed
    empirically: WTI's 652,495 combo rows reduce to 4,817 unique keys, a
    135x reduction) with zero behavior change versus the original per-row
    loop -- same classify_combo/parse/resolve_legs/_legs_to_symbols calls,
    just batched. Returns {"reject_reason": None or a reason string} plus,
    on success, {"bucket": ..., <flattened leg/identity fields>}."""
    ...
    leg_symbols = _legs_to_symbols(resolved, [0])
    ...
    if combo_type in ("plain", "labeled_intercommodity", "crack"):
        ...
        return {"reject_reason": None, "bucket": "spread",
                "spread_symbol": raw_symbol,
                "near_root": near["root"], "near_contract_symbol": near["contract_symbol"],
                "near_expiry_year": near["expiry_year"], "near_expiry_code": near["expiry_code"],
                "far_root": far["root"], "far_contract_symbol": far["contract_symbol"],
                "far_expiry_year": far["expiry_year"], "far_expiry_code": far["expiry_code"]}
    if combo_type in ("butterfly", "butterfly3"):
        near, mid, far = leg_symbols
        return {"reject_reason": None, "bucket": "butterfly",
                "butterfly_symbol": raw_symbol,
                "near_root": near["root"], "near_contract_symbol": near["contract_symbol"],
                "near_expiry_year": near["expiry_year"], "near_expiry_code": near["expiry_code"],
                "mid_root": mid["root"], "mid_contract_symbol": mid["contract_symbol"],
                "mid_expiry_year": mid["expiry_year"], "mid_expiry_code": mid["expiry_code"],
                "far_root": far["root"], "far_contract_symbol": far["contract_symbol"],
                "far_expiry_year": far["expiry_year"], "far_expiry_code": far["expiry_code"]}
    if combo_type in ("condor", "condor4"):
        near, mid1, mid2, far = leg_symbols
        return {"reject_reason": None, "bucket": "condor",
                "condor_symbol": raw_symbol,
                "near_root": near["root"], "near_contract_symbol": near["contract_symbol"],
                "near_expiry_year": near["expiry_year"], "near_expiry_code": near["expiry_code"],
                "mid1_root": mid1["root"], "mid1_contract_symbol": mid1["contract_symbol"],
                "mid1_expiry_year": mid1["expiry_year"], "mid1_expiry_code": mid1["expiry_code"],
                "mid2_root": mid2["root"], "mid2_contract_symbol": mid2["contract_symbol"],
                "mid2_expiry_year": mid2["expiry_year"], "mid2_expiry_code": mid2["expiry_code"],
                "far_root": far["root"], "far_contract_symbol": far["contract_symbol"],
                "far_expiry_year": far["expiry_year"], "far_expiry_code": far["expiry_code"]}
    ...
    # combo_type == "pack"
    near, far = leg_symbols
    return {"reject_reason": None, "bucket": "pack",
            "pack_symbol": raw_symbol, "pack_type": pack_type,
            "near_root": near["root"], "near_contract_symbol": near["contract_symbol"],
            "near_expiry_year": near["expiry_year"], "near_expiry_code": near["expiry_code"],
            "far_root": far["root"], "far_contract_symbol": far["contract_symbol"],
            "far_expiry_year": far["expiry_year"], "far_expiry_code": far["expiry_code"]}
...
def _join_bucket(combo_rows_full: pl.DataFrame, keys_list: list, columns: list, asset: str) -> pl.DataFrame:
    ...
    table, instead of building it row-by-row. `_combo_pl_df`'s existing
    empty-schema handling is reused so a bucket with zero successful
    resolutions still produces a correctly-typed zero-row frame."""
    if not keys_list:
...
def transform_asset(asset: str, root: str, exchange: str) -> dict:
    ...
    outright_rows = (
        ...
        # per-row `map_elements` Python callback (identical result --
        # `expiry_code` is a pure function of `maturity_month` alone).
        .join(MONTH_LOOKUP_DF, on="maturity_month", how="left")
        # Vectorized: plain polars string concatenation instead of a
        # per-row `map_elements` call into `_contract_symbol` -- `root`/
        # `exchange` are asset-level constants here, so this is exactly
    ...
    # Vectorized: classify_combo/leg-parsing/resolve_legs/_legs_to_symbols
    # are pure functions of (raw_symbol, maturity_year, maturity_month, root)
    # -- called once per UNIQUE key here (not once per row), then joined
    # back onto the full row set in polars. Same functions, same results,
    # just batched (see _resolve_combo_key's docstring for the empirically
    # confirmed ~135x row/key compression on WTI).
    ...
    if combo_rows_full is not None:
        ...
        # Reject-reason counters need to be ROW counts (matching the
        # original per-row loop's semantics), not unique-key counts -- a
        # left-join of the full row set against the small per-key reject
        # table, then filtering/counting, expands each key back out to
        # however many actual rows shared it.
        if reject_keys:
...
def _write_parquet_atomic(df: pl.DataFrame, path: Path):
    """Write to a temp file then `os.replace()` into the real path -- never
    asks the OS to overwrite a path a reader may have memory-mapped in the
    same process (a real, reproduced Windows file-locking failure once
    `term_structure_spreads.parquet` grew large, see module docstring)."""
...
def _merge_append(new_df: pl.DataFrame, path: Path, dedup_subset: list):
    ...
    if path.exists():
        ...
        # (datetime64[ns]) or by an earlier run of this module -- both
        # read back as pl.Datetime, but harmonizing explicitly avoids ever
        # relying on that matching by coincidence (the exact
        # `TypeError: Cannot compare Timestamp with datetime.date` this
        # project hit live comes from the equivalent pandas failure mode --
        # caught on the very first asset run through the full pipeline).
