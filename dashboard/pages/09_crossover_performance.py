...
all-pairs summary table) is one selected asset's own return stream
...
@st.cache_data(ttl=1200)
def _all_pair_summary(_asset):
    signals = all_pair_signals(close, vol, target_vol=co.DEFAULT_TARGET_VOL)
    rows = []
    for name, signal in signals.items():
        turnover = co.annualized_turnover(signal)
        gross = backtest_signal_per_asset(signal, returns, frequency=co.FREQUENCY)[_asset]
        net = backtest_signal_per_asset(signal, returns, frequency=co.FREQUENCY, cost_bps=cost_bps)[_asset]
...
summary = _all_pair_summary(asset)
render_key_takeaways([
    "All three pairs reported as parallel Books, **no auto-picked winner** — "
    "same discipline as breakout's two systems.",
    "Pooled-book finding, for context: **50/200 (golden cross)** was the most "
    "consistent across the full universe — positive throughout train/validation/test. "
    "50/100 faded badly out-of-sample; 100/200 sign-flipped sharply in validation. "
    f"**{asset}**'s own numbers below may or may not follow that pattern.",
    f"**{asset}**'s turnover ranges {summary['Annualized turnover'].min():.1f}-"
    f"{summary['Annualized turnover'].max():.1f}x annualized across the three pairs.",
])
...
st.subheader(f"All Pairs — {asset} Gross/Net Sharpe")
...
asset_returns = backtest_signal_per_asset(signal, returns, frequency=co.FREQUENCY, cost_bps=cb)[asset]
stats_by_period = {k: performance_stats(v) for k, v in zip(("train", "validation", "test"), train_validation_test_split(asset_returns))}

st.subheader(f"Tearsheet — {asset}, {pair.replace('_', '/')}, {gross_net}")
